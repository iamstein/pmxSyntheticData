...
ROLES <- pmx_roles(
  ...
  cens     = NULL,           # e.g. "CENS"  (Monolix: 1 = below, -1 = above)
...
if (requireNamespace("ggplot2", quietly = TRUE)) {
  # `dvid` may name more than one column (here YTYPE and NAME); the first is the
  # endpoint key, so index it with [[1]] rather than the whole vector.
  endpoint_col <- if (is.null(roles$dvid)) NULL else roles$dvid[[1]]
  obs <- function(d, label) {
    ...
    data.frame(dataset = label,
               ...
               endpoint = if (is.null(endpoint_col)) "DV" else
                 as.character(d[[endpoint_col]][keep]))
  }
