...
ROLES <- pmx_roles(
  ...
  amt      = "AMT",
  evid     = "EVID",
  dvid     = "YTYPE",         # numeric endpoint key; NULL if a single endpoint
  cmt      = "CMT",
  mdv      = "MDV",
  ...
  # keep = copied verbatim from the same subject that supplied the doses, so it
  # stays coherent with them. Put assigned, subject-defining values here: the
  # character endpoint label NAME that is redundant with the numeric YTYPE
  # above, and any treatment arm / dose group you want kept faithful, e.g.
  # keep = c("NAME", "ARM"). List only columns your data actually has.
  keep     = "NAME"
