...
# Optional cleanup here: filter to one study, fix units, recode a column.
# Pooling two assays with different limits trips the censoring check, so filter
# to a single study if that applies, e.g. raw <- raw[raw$STUDYID == "X", ].
roles <- ROLES
