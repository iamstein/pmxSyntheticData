...
| `REV-023` | API / Governance | The DP engines' unaudited status (`README.Rmd` "Maintenance status": complete and tested, not under active development, known open findings, not independently privacy-audited) is documented but not enforced. `synpmx_calibrated()` and `synpmx_empirical()` are exported functions exactly as frictionless to call as `synpmx_avatar()` — nothing at the call site distinguishes "principled demonstration of the privacy/utility tradeoff" from "production release mechanism." The durable contribution of the DP path is the reasoning that ships alongside it — the cohort-size feasibility analysis (`REV-002`, Part 3 above, `vignettes/articles/feasibility.Rmd`) and the trust-boundary decision rule (`README.Rmd`, `synpmx-privacy.Rmd`) — not the specific mechanism implementation, which still carries open P0/P1 findings (`REV-003` unaccounted `stop()` channel, `REV-004` decorative `delta`). Risk shows up at the moment someone decides to ship the output, and prose alone does not stop that moment from arriving silently in a script or pipeline. | A session-level opt-in gate, not a per-call default-off argument (too easy to flip `TRUE` once by copy/paste and never revisit) and not a `warning()` (too easy to suppress or scroll past in a script). Add an exported `synpmx_enable_dp_engines()` that sets an in-session flag (an option, not persisted to disk or across restarts); `synpmx_calibrated()` and `synpmx_empirical()` check it on entry and `stop()` with the audit-status text and a pointer to `vignette("synpmx-privacy")` if it was never called. `backend = "public"` makes no DP claim already and should stay exempt from the gate. Open before implementing: (a) whether calling the gate function should itself echo back what it is unlocking (self-documenting in a transcript/log), so the acknowledgment is visible even without reading the source; (b) whether it needs a matching `synpmx_disable_dp_engines()` or is intentionally sticky for the rest of the session; (c) `tests/testthat/test-synpmx-api.R`, `scripts/demo_nlmixr2data.R`, and `scripts_private/try_dp_calibrated.R` all need the gate call added wherever they exercise these two functions. | open |
